...
class MaskFormer(nn.Module):
    ...
    def forward(self, batched_inputs, keys, get_train=False, get_eval=False, raw_sem_seg=False):
        return self.forward_base(batched_inputs, keys, get_train=get_train, get_eval=get_eval, raw_sem_seg=raw_sem_seg)
    ...
    def forward_base(self, batched_inputs, keys, get_train=False, get_eval=False, raw_sem_seg=False):
        ...
        images = batched_inputs[keys[0]].to(self.device)
        images = images - self.pixel_mean[None]
        images = images / self.pixel_std[None]
        images = ImageList.from_tensors([i for i in images], self.size_divisibility)
        ...
        if get_eval:
            ...
            processed_results = []
            if 'width' in batched_inputs:
                widths = [w.item() for w in batched_inputs['width']]
                heights = [h.item() for h in batched_inputs['height']]
            else:
                widths = [images.image_sizes[i][1] for i in range(len(images))]
                heights = [images.image_sizes[i][0] for i in range(len(images))]
                
            for mask_cls_result, mask_pred_result, height, width in zip(
                    mask_cls_results, mask_pred_results, heights, widths
            ):
